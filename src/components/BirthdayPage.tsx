...
import { Sparkles, Heart, ChevronDown, Lock, GraduationCap, BookOpen, Award, Quote } from 'lucide-react';
...
export function BirthdayPage() {
  // Mode flag: set to false whenever you want to switch back to Divu's Birthday Vault
  const IS_TEACHERS_DAY_MODE = true;

  // In Teachers' Day mode, PIN lock is skipped
  const [isPinUnlocked, setIsPinUnlocked] = useState<boolean>(() => IS_TEACHERS_DAY_MODE ? true : false);
  ...
  const [showIntro, setShowIntro] = useState<boolean>(() => {
    if (IS_TEACHERS_DAY_MODE) return false;
  ...
  useEffect(() => {
    if (IS_TEACHERS_DAY_MODE) return;
  ...
  // If locked, present the Birthday PIN Lock screen with full theme capabilities (skipped in Teachers' Day mode)
  if (!isPinUnlocked && !IS_TEACHERS_DAY_MODE) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background text-foreground transition-colors duration-500 overflow-x-hidden selection:bg-brand/30 selection:text-brand relative">
      ...
      {/* Dynamic Birthday Heart & Kinetic Typography Gateway (skipped in Teachers' Day mode) */}
      {!IS_TEACHERS_DAY_MODE && (
        <AnimatePresence>
          {showIntro && (
            <motion.div
      ...
      {/* Birthday Specific Decor Elements (Preserved for Birthday mode) */}
      {!IS_TEACHERS_DAY_MODE && (
        <>
      ...
      {IS_TEACHERS_DAY_MODE ? (
        /* TEACHERS' DAY 2026 - DEDICATED SINGLE SCREEN TRIBUTE */
        <section className="relative min-h-[92vh] flex flex-col items-center justify-center text-center px-4 sm:px-8 max-w-4xl mx-auto w-full py-12 sm:py-16">
          {/* Top Celebration Badges */}
          <div className="flex flex-wrap items-center justify-center gap-2.5 sm:gap-3 mb-5 sm:mb-6">
            ...
            >
              ...
              <span>5th September 2026 • Teachers' Day Special</span>
            ...
            <motion.div
              ...
              className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 text-xs sm:text-sm font-medium text-foreground/85 backdrop-blur-sm shadow-sm"
            >
              <Award className="w-3.5 h-3.5 text-amber-400" />
              <span>Balwant College, Vita</span>
            </motion.div>
          </div>

          {/* Grand Teachers' Day Main Heading */}
          <motion.h1
            initial={{ opacity: 0, y: 25 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.15 }}
            className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-display font-black text-foreground tracking-tight leading-[1.08] mb-6 drop-shadow-sm"
          >
            Happy Teachers' Day! <br />
            <span className="text-brand bg-gradient-to-r from-amber-400 via-brand to-yellow-500 bg-clip-text text-transparent drop-shadow-md">
              To Our Inspiring Teachers
            </span>
            <span className="inline-block ml-3 text-3xl sm:text-5xl md:text-6xl animate-pulse">🎓✨</span>
          </motion.h1>

          {/* Heartfelt Note Card for Balwant College, Vita Teachers */}
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.25 }}
            className="w-full max-w-2xl mx-auto my-3 sm:my-5 p-6 sm:p-8 rounded-2xl bg-white/[0.04] border border-white/15 backdrop-blur-xl shadow-2xl text-left relative overflow-hidden group hover:border-brand/35 transition-all"
          >
            {/* Soft internal glows */}
            ...
            <div className="flex items-center gap-2.5 mb-4 text-brand">
              <BookOpen className="w-5 h-5" />
              <span className="text-xs sm:text-sm font-bold tracking-wider uppercase">Tribute & Gratitude Note</span>
            </div>

            <p className="text-foreground/90 text-sm sm:text-base md:text-lg leading-relaxed font-normal mb-4">
              To the respected teachers and professors of <strong className="text-brand font-semibold">Balwant College, Vita</strong> — thank you for being our mentors, motivators, and guiding lights.
            </p>

            <p className="text-muted-foreground text-xs sm:text-sm md:text-base leading-relaxed mb-6">
              You teach far beyond textbooks and classrooms. You ignite curiosity, cultivate integrity, and instill in us the confidence to aim higher. Every milestone achieved by your students carries the imprint of your patience, wisdom, and selfless encouragement.
            </p>

            <div className="pt-4 border-t border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs sm:text-sm">
              <div className="flex items-center gap-2 text-foreground/75 italic">
                <Quote className="w-3.5 h-3.5 text-amber-400 shrink-0" />
                <span>"A teacher affects eternity; he can never tell where his influence stops."</span>
              </div>
              <span className="text-brand font-medium shrink-0 flex items-center gap-1.5">
          ...
          <motion.div
            ...
            className="flex flex-wrap justify-center items-center gap-2 sm:gap-3 max-w-2xl"
          >
            ...
            <span className="px-3.5 py-1.5 rounded-full bg-white/5 border border-white/10 text-[11px] sm:text-xs font-semibold text-foreground/85 backdrop-blur-sm shadow-sm flex items-center gap-1.5">
              🌱 Shaping Futures 2026
            </span>
          </motion.div>
        </section>
      ) : (
        /* ORIGINAL BIRTHDAY HERO SECTION */
      ...
      {/* SECTIONS FROM THE TREE ONWARDS - PRESERVED FOR LATER USE (HIDDEN IN TEACHERS' DAY MODE) */}
      {!IS_TEACHERS_DAY_MODE && (
        <>
