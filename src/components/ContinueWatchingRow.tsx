...
export function ContinueWatchingRow() {
  ...
  return (
    <div className="mb-12 md:mb-16">
      <h2 className="text-xl md:text-2xl font-bold text-cv-cream mb-4 font-serif">Continue Watching</h2>
      <div 
      ...
      >
        {continueWatching.map((item) => (
          <div 
            ...
            className="flex-shrink-0 w-[240px] md:w-[320px] aspect-video rounded-md bg-white/5 relative border border-white/5 group cursor-pointer snap-start overflow-hidden transition-all hover:scale-[1.02] hover:border-cv-gold/50"
            onClick={() => {
          ...
          >
            ...
            <div className="absolute inset-0 flex flex-col justify-end p-4 bg-gradient-to-t from-black via-black/40 to-transparent">
              <div className="flex items-center justify-center absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="w-12 h-12 rounded-full bg-cv-gold/20 flex items-center justify-center backdrop-blur-md border border-cv-gold">
                  <Play className="w-6 h-6 text-cv-gold fill-current ml-1" />
                </div>
              </div>
              ...
              {(item.media_type === 'tv') && item.season_number && item.episode_number && (
                <p className="text-cv-gold text-xs font-medium z-10">S{item.season_number} E{item.episode_number}</p>
              )}
              {item.media_type === 'anime' && item.episode_number && (
                <p className="text-cv-gold text-xs font-medium z-10">Continue E{item.episode_number}</p>
              )}
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/20">
              <div 
                className="h-full bg-cv-gold shadow-[0_0_8px_rgba(212,168,83,0.8)]"
