...
export function FilterBar({ onFilterChange, defaultType = 'movie' }: FilterBarProps) {
  ...
  const DropdownItem = ({ active, onClick, children }: any) => (
    <button
      ...
      className={cn(
        ...
        active ? "text-cv-gold font-bold bg-cv-gold/5" : "text-cv-cream"
      )}
    >
      {children}
    </button>
  ...
  return (
    <div className="px-4 md:px-10 py-6 flex flex-wrap items-center gap-4 md:gap-8 border-b border-cv-gold/10 relative z-30">
      <div className="flex items-center gap-2">
        <span className="text-cv-slate text-sm font-medium">Type:</span>
        <div className="relative">
          <button 
            onClick={() => setShowType(!showType)} 
            onBlur={() => setTimeout(() => setShowType(false), 200)}
            className="glass-panel px-4 py-2 rounded-full text-sm font-medium text-cv-cream border border-white/10 hover:border-cv-gold flex items-center gap-2 transition-colors"
          >
          ...
          <AnimatePresence>
            {showType && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                className="absolute top-full left-0 mt-2 w-40 glass-panel rounded-lg py-2 border border-cv-gold/20 shadow-xl"
              >
                {TYPES.map(t => (
      ...
      <div className="flex items-center gap-2">
        <span className="text-cv-slate text-sm font-medium">Country:</span>
        <div className="relative">
          <button 
          ...
          <AnimatePresence>
            {showCountry && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 10 }}
                className="absolute top-full left-0 mt-2 w-48 glass-panel rounded-lg py-2 border border-cv-gold/20 shadow-xl"
              >
                {COUNTRIES.map(c => (
      ...
      <div className="flex items-center gap-2">
        <span className="text-cv-slate text-sm font-medium">Streaming On:</span>
        <div className="relative">
          <button 
          ...
          <AnimatePresence>
            {showProviders && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 10 }}
                className="absolute top-full left-0 mt-2 w-56 max-h-64 overflow-y-auto glass-panel rounded-lg py-2 border border-cv-gold/20 shadow-xl scrollbar-hide"
              >
