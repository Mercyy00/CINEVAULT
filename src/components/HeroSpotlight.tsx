...
export function HeroSpotlight({
  ...
  useEffect(() => {
    ...
    if (heroCache[type] && heroCache[type].length > 0) {
          ...
          if (type === 'anime') {
            ...
            if (data.data) {
              results = data.data.map((item: any) =>
                kitsuApi.mapKitsuToInternal(item, data.included)
              );
            }
          } else {
  ...
  const typeLabel = type === 'anime' ? 'Anime Series' : type === 'tv' ? 'TV Spotlight' : 'Movie Spotlight';
