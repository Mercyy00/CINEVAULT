    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -50 }}
          className="fixed inset-0 z-[100] bg-cv-bg/95 backdrop-blur-2xl overflow-y-auto"
        >
          <div className="min-h-screen flex flex-col p-6 md:p-12 relative">
            <button 
              onClick={onClose}
              className="absolute top-6 right-6 p-2 text-cv-cream/50 hover:text-cv-cream hover:bg-white/10 rounded-full transition-colors z-10"
            >
              <X className="w-8 h-8" />
            </button>
              ...
              <AnimatePresence mode="wait">
                {!selectedMood ? (
                  <motion.div
                    ...
                    className="w-full"
                  >
                    <h2 className="text-4xl md:text-6xl font-serif font-bold text-cv-cream text-center mb-16">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                      {MOODS.map((mood, idx) => (
                        ...
                        >
                          <span className="text-xl md:text-2xl font-bold text-cv-cream group-hover:text-cv-gold transition-colors">{mood.label}</span>
                        </motion.button>
                      ))}
                    </div>
                  </motion.div>
                  ...
                  >
                    <div className="text-center mb-12">
                      <h3 className="text-cv-gold uppercase tracking-widest text-sm font-bold mb-2">Curated for your mood</h3>
                      <h2 className="text-3xl md:text-5xl font-serif font-bold text-cv-cream">{selectedMood.label}</h2>
                    </div>

                    {loading ? (
                      <div className="flex flex-col items-center justify-center py-20">
                        <div className="w-16 h-16 border-4 border-cv-gold/30 border-t-cv-gold rounded-full animate-spin mb-6" />
                        <p className="text-cv-slate animate-pulse">Finding perfect matches...</p>
                      </div>
                    ) : (
                      <>
                            ...
                            >
                              <div className="aspect-[2/3] rounded-xl overflow-hidden mb-3 relative">
                                ...
                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                  <div className="w-12 h-12 rounded-full bg-cv-gold flex items-center justify-center text-cv-gold-content">
                                    <Play className="w-6 h-6 ml-1" />
                                  </div>
                                </div>
                              </div>
                              <h4 className="font-bold text-cv-cream truncate">{movie.title}</h4>
                              <div className="flex items-center gap-2 text-sm text-cv-slate truncate">
                        ...
                        <div className="flex justify-center">
                          <button
                            onClick={handleReset}
                            className="flex items-center gap-2 px-8 py-3 bg-white/10 hover:bg-white/20 text-cv-cream rounded-full font-bold transition-all hover:scale-105"
                          >
