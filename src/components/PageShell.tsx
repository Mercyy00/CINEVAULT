    ...
    <div className="pb-12 min-h-screen">
      ...
      <div className={`max-w-[1600px] mx-auto px-4 md:px-10 ${isSearch ? 'pt-24' : 'pt-8'}`}>
        <div className="mb-8 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
          <h1 className="text-3xl font-serif font-bold text-cv-cream">{isSearch ? title : "Explore " + title}</h1>
        </div>
        ...
        {!isSearch && getPills().length > 0 && (
          <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-4 mb-8" role="tablist">
            {getPills().map((pill) => (
              <button
                ...
                className={`px-5 py-2.5 rounded-full whitespace-nowrap font-medium transition-all ${
                  activePill === pill.id 
                    ? 'bg-cv-gold text-cv-gold-content shadow-[0_0_15px_rgba(212,168,83,0.3)]' 
                    : 'bg-white/5 text-cv-slate hover:bg-white/10 hover:text-white border border-white/10'
                }`}
              >
                {pill.label}
              </button>
            ...
            {filters.type !== 'anime' && (
                ...
                className="px-5 py-2.5 rounded-full whitespace-nowrap font-medium transition-all bg-white/5 text-cv-slate hover:bg-white/10 hover:text-white border border-white/10 flex items-center gap-2 ml-auto"
                >
        ...
        {error ? (
          <div className="py-20 flex justify-center">
            <div className="glass-panel border-red-500/30 p-8 rounded-2xl max-w-lg text-center backdrop-blur-xl">
              <h3 className="text-2xl font-serif text-white mb-4">Oops!</h3>
            ...
            {hasMore && !isSearch && !loading && movies.length > 0 && (
              <div className="mt-12 flex justify-center pb-12">
                <button
                  ...
                  className="px-8 py-3 glass-panel border border-cv-gold/30 text-cv-gold font-bold rounded-lg hover:bg-cv-gold/10 transition-colors disabled:opacity-50"
                >
          ...
          <div className="text-center py-20 text-cv-slate text-lg">
            No results found.
          </div>
        )}
      </div>
        ...
        {isFilterDrawerOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed top-0 right-0 h-full w-full max-w-sm glass-panel z-[260] border-l border-white/10 p-6 flex flex-col"
            >
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-serif font-bold text-white">Advanced Filters</h2>
                <button onClick={() => setIsFilterDrawerOpen(false)} className="p-2 bg-white/5 hover:bg-white/10 rounded-full transition-colors text-white">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-8 pr-2">
                <div>
                  <label className="block text-sm font-medium text-cv-slate mb-2">Year Range</label>
                  <div className="flex items-center gap-4">
                    <select
                      value={advFilters.minYear}
                      onChange={e => setAdvFilters(p => ({ ...p, minYear: e.target.value }))}
                      className="w-full bg-black/50 border border-white/10 rounded-lg px-3 py-2 text-white outline-none focus:border-cv-gold"
                    >
                      {Array.from({length: 45}, (_, i) => 2024 - i).map(y => (
                        <option key={`min-${y}`} value={y}>{y}</option>
                      ))}
                    </select>
                    <span className="text-cv-slate">to</span>
                    <select
                ...
                <div>
                  <label className="block text-sm font-medium text-cv-slate mb-2 flex justify-between">
                    <span>Minimum Rating</span>
                    <span className="text-cv-gold font-bold">{advFilters.minRating}+</span>
                  </label>
                  <input 
                    type="range" 
                    min="1" 
                    ...
                    className="w-full accent-cv-gold"
                  />
                </div>

                <div>
                  <label className="block text-sm font-medium text-cv-slate mb-3">Sort By</label>
                  <div className="space-y-3">
                    {[
                      ...
                      <label key={opt.val} className="flex items-center gap-3 cursor-pointer group">
                        <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${advFilters.sortBy === opt.val ? 'border-cv-gold' : 'border-white/30 group-hover:border-white/60'}`}>
                          {advFilters.sortBy === opt.val && <div className="w-2.5 h-2.5 rounded-full bg-cv-gold" />}
                        </div>
                        <span className={`transition-colors ${advFilters.sortBy === opt.val ? 'text-white font-medium' : 'text-cv-slate group-hover:text-white/80'}`}>{opt.label}</span>
                        <input 
                          type="radio" 
                          name="sortBy"
                          value={opt.val}
              ...
              <div className="pt-6 mt-6 border-t border-white/10">
                <button 
                  ...
                  className="w-full py-4 bg-cv-gold text-cv-gold-content font-bold rounded-xl hover:bg-cv-gold-light transition-colors shadow-[0_0_20px_rgba(212,168,83,0.3)]"
                >
                  Apply Filters
                </button>
              </div>
