          ...
          >
            <div className="flex items-center gap-2 sm:gap-4 min-w-0">
              ...
              <div className="hidden lg:block min-w-0">
                <h1 className="text-lg sm:text-xl font-bold text-foreground shadow-black drop-shadow-md truncate">{movie.title}</h1>
                {type === 'tv' && selectedEpisode && (
                  <p className="text-xs sm:text-sm text-brand tracking-wide font-medium truncate">S{selectedSeason} E{selectedEpisode.episode_number} - {selectedEpisode.name}</p>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full bg-card/80 hover:bg-brand/20 border border-white/10 text-[11px] sm:text-xs font-bold text-foreground backdrop-blur-md transition-colors cursor-pointer"
              >
                <Signal className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-brand" />
                <span className="max-w-[90px] sm:max-w-none truncate">{selectedServer.name}</span>
                <span className="text-[9px] sm:text-[10px] px-1 py-0.5 rounded bg-brand/20 text-brand uppercase font-mono">{selectedServer.quality}</span>
              </button>
            </div>
          </motion.div>
        )}
